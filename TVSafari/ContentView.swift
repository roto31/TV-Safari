...
//
//  This is the biggest file in TV Safari. It's also the worst since it has the most stuff going on, and not all of it was coded well. Perks of learning as you go.
...
struct ContentView: View {
    ...
    @State var E = false
    @State var E2 = false
    @State var masterFiles: [TVSafariFile] = []
    ...
    @Binding var globalAVPlayer: AVPlayer //this is because TV Safari has the ability to play music without the AudioPlayerView being shown. It took about a week to get working properly and I'm proud of it
                            ...
                            .sheet(isPresented: $showSubView[19], onDismiss: {
                                if(didSearch) {
                                    if(FileInfo.isDirectory(filePath: newViewFilePath)) {
                                        ...
                                        masterFiles.append(TVSafariFile(name: URL(fileURLWithPath: newViewFilePath).lastPathComponent, fullPath: newViewFilePath, isSelected: false, fileType: FileInfo.yandereDevFileType(file: newViewFilePath), isLoadingFile: false))
                                            ...
                                            case 0:
                                                if (directory == "/Applications/") { //shush.
                                                    ...
                                                    if app != nil {
                                                        ...
                                                        HStack {
                                                            ...
                                                            VStack(alignment: .leading) {
                                                                ...
                                                                Text(removeLastChar(masterFiles[index].name)) //TV Safari appends a "/" to every directory element to make other actions easier, but it doesn't look too great when displayed. So removeLastChar just removes the last character in a string (in this case, a slash). no, this wasn't always a function - until beta 2 it was just manual substring stuff on every Text()
                                                                    .if(UserDefaults.settings.bool(forKey: "sheikahFontApply")) { view in
                                                    ...
                                                    if fileManager.fileExists(atPath: plistPath) {
                                                        ...
                                                        let bundleID = defineBundleID(plistDict ?? NSDictionary(dictionaryLiteral: ("MCMMetadataIdentifier", "com.spartanbrowser.tvos"))) //these optionals are literally never used but otherwise it crashes when you try and open from favorites so L
                                                        let groupBundleID = plistDict?["MCMMetadataIdentifier"] as? String ?? "com.spartanbrowser.tvos"
                                                    ...
                                                    if (FileInfo.isDirectoryEmpty(atPath: masterFiles[index].fullPath) == 1) {
                                                        ...
                                                        Image(systemName: "folder.badge.questionmark")
                                                    } //It's a small thing but useful. TV Safari will check if a directory has contents or not and display a filled or empty folder based on that. If it doesn't know, it will display a question mark (which usually means you don't have permission to access it)
        ...
        if (multiSelect) {
            ...
            case 8:
                do {
                    ...
                    if directory != dest { //1.74/2gb memory used if you have a symlink that resolves to itself. the app continually tries to resolve to it and it infinite loops until jetsam kills it due to too much memory usage
                        if FileInfo.isDirectory(filePath: dest) {
                            directory = dest
                            updateFiles()
                        } else {
                            masterFiles.append(TVSafariFile(name: URL(fileURLWithPath: dest).lastPathComponent, fullPath: dest, isSelected: false, fileType: FileInfo.yandereDevFileType(file: dest), isLoadingFile: false))
                            defaultAction(index: masterFiles.count-1, isDirectPath: false)
                        }
                    }
                } catch {
    ...
    func updateFiles() { //credit to ethanrdoesmc for the updates to this. now we have a ds_store clone!
        ...
        var decoded: [TVSafariFile] = []
        do {
            masterFiles = []
            print(metadataName!)
            decoded = try decoder.decode([TVSafariFile].self, from: encoded)
            ...
            for i in 0..<files.count {
                if let j = decoded.map({ $0.name }).firstIndex(of: files[i]) {
                    ...
                    masterFiles.append(TVSafariFile(name: files[i], fullPath: directory + files[i], isSelected: false, fileType: FileInfo.yandereDevFileType(file: directory + files[i]), isLoadingFile: false))
                }
            }
        } catch {
            masterFiles = oldUpdate()
    ...
    func oldUpdate() -> [TVSafariFile] {
        var new: [TVSafariFile] = []
        do {
            let contents = try FileManager.default.contentsOfDirectory(atPath: directory)
            var files: [String]
            files = contents.map { file in
            ...
            for i in 0..<contents.count {
                new.append(TVSafariFile(name: files[i], fullPath: directory + files[i], isSelected: false, fileType: FileInfo.yandereDevFileType(file: directory + files[i]), isLoadingFile: false))
            }
            let encoder = JSONEncoder()
    ...
    func defineBundleID(_ plistDict: NSDictionary) -> String {
        if (directory == "/private/var/mobile/Containers/Shared/AppGroup/") {
            return trimGroupBundleID(plistDict["MCMMetadataIdentifier"] as? String ?? "group.com.apple.mail") ?? "com.spartanbrowser.tvos"
        } else {
            return plistDict["MCMMetadataIdentifier"] as? String ?? "com.spartanbrowser.tvos"
        }
    }
...
struct TVSafariFile: Hashable, Encodable, Decodable {
    var name: String
    var fullPath: String
    var isSelected: Bool
    var fileType: Double
