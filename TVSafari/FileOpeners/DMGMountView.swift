    ...
    var body: some View {
		/*VStack {
			...
			Button(action: {
				...
				if TVSafari.fileManager.fileExists(atPath: dmgFilesAreAvailableAt, isDirectory: &isDir) {
					do {
						try TVSafari.fileManager.createDirectory(atPath: dmgFilesAreAvailableAt, withIntermediateDirectories: true, attributes: nil)
					} catch {
